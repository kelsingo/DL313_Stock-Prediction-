...
def load_model():
    ...
    model = tf.keras.models.load_model("AAPL_1st.keras")
    print("Model ready!")

class InputData(BaseModel):
    data: List[List[float]]

def MinMax_normalize(X_test):
  ...
  return X_test_norm, max_feature - min_feature, min_feature
...
async def predict(request: InputData):
    async with model_semaphore:
        ...
        input_array, max_diff, min_val = MinMax_normalize(input_array)
        
        loop = asyncio.get_event_loop()
        result = await loop.run_in_executor(executor, model.predict, input_array)
        result = result * max_diff + min_val  # Denormalize the output
